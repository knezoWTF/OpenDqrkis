package xyz.dqrkis.gui.components;

import xyz.dqrkis.Dqrkis;
import xyz.dqrkis.gui.Window;
import xyz.dqrkis.gui.components.settings.*;
...
public final class ModuleButton {
	...
	public ModuleButton(Window parent, Module module, int offset) {
		...
		for (Setting<?> setting : module.getSettings()) {
			if (setting instanceof BooleanSetting booleanSetting)
				...
				settings.add(new ItemSettingBox(this, itemSetting, settingOffset));
			else if (setting instanceof ColorSetting colorSetting)
				settings.add(new ColorPickerBox(this, colorSetting, settingOffset));
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		Color toColor = module.isEnabled() ? Utils.getMainColor(255, Dqrkis.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module)) : Color.WHITE;
		...
		if (parent.moduleButtons.get(parent.moduleButtons.size() - 1) != this) {
			...
			context.fillGradient(parent.getX(), parent.getY() + offset, parent.getX() + 2, parent.getY() + parent.getHeight() + offset, Utils.getMainColor(255, Dqrkis.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module)).getRGB(), Utils.getMainColor(255, Dqrkis.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module) + 1).getRGB());
		} else {
			...
			RenderUtils.renderRoundedQuad(context, Utils.getMainColor(255, Dqrkis.INSTANCE.getModuleManager().getModulesInCategory(module.getCategory()).indexOf(module)), parent.getX(), parent.getY() + offset, parent.getX() + 2, parent.getY() + (parent.getHeight() - 1) + offset, 0, 0, extended ? 0 : 2, 0, 50);
		}
